  ...
  return (
    <nav className='fixed top-0 left-0 w-full z-50'>
      <div className='flex justify-between items-center max-w-6xl mx-auto md:my-2 bg-stone-950/30 p-4 md:rounded-xl backdrop-blur-lg'>
        <div className='text-white font-semibold uppercase text-lg'><a href="/">Jason Root</a></div>
        
        <div className='hidden md:flex space-x-8'>
        {LINKS.map((link, index) => (
              <a 
              key={index} href={link.id} onClick={handleLinkClick} className=' text-white transition duration-300 hover:text-stone-400'>
                {link.name}
              </a>
            ))}
        </div>
